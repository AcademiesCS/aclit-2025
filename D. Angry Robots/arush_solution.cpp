...
{
    float v, th, x, y, t; cin >> v >> th >> x >> y >> t;
    float vx = v * cos(PI / 180.0 * th);
    float vy = v * sin(PI / 180.0 * th);
    float dx = vx * t;
    float dy = vy * t - 0.5 * 9.80665 * t * t;

    cout << fixed << setprecision(8) << x + dx << " " << y + dy << endll;
}
